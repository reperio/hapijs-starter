...
class Server {
    constructor(options) {
        ...
        const hapiConfig = Object.assign({}, { port: this.config.port, host: this.config.host, routes: Object.assign({}, {cors: this.config.cors}, this.config.additionalOptions != null && this.config.additionalOptions.routes != null ? this.config.additionalOptions.routes : null), cache: { engine: require('catbox-memory'), name: 'memory' } }, this.config.additionalOptions);
    ...
    strategy(name, schema, options) {
        this.server.auth.strategy(name, schema, options);
    }

    async startServer() {
        const logTransports = [].concat(this.config.logAddtionalLoggerTransports);
        const traceTransports = [].concat(this.config.logAddtionalTraceTransports);
        const activityTransports = [].concat(this.config.logAddtionalActivityTransports);
        ...
        if (this.config.logDefaultActivityTransport) {
            activityTransports.push(new (winston.transports.DailyRotateFile)({
                filename: `${this.config.logDirectory}/activity.log`,
                datePattern: 'yyyy-MM-dd.',
                createTree: true,
                prepend: true,
                level: this.config.logLevel,
        ...
        if (traceTransports.length > 0) {
            this.appTraceLogger = new (winston.Logger)({
        ...
        if (activityTransports.length > 0) {
            this.appActivityLogger = new (winston.Logger)({
        ...
        this.server.app.logger = this.appLogger;
        this.server.app.traceLogger = this.appTraceLogger;
        this.server.app.activityLogger = this.appActivityLogger;
